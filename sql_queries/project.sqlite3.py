...
def main() -> None:
    ...
    print(f"Created or verified data folder: {DATA_FOLDER}")
    ...
    try:
        ...
        execute_sql_file(connection, SQL_CREATE_FOLDER.joinpath('01_project_drop_tables.sql'))
        execute_sql_file(connection, SQL_CREATE_FOLDER.joinpath('02_project_create_tables.sql'))
        # Debug: check if tables exist immediately after creation
        cursor = connection.cursor()
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table';")
        tables = cursor.fetchall()
        print("Tables in DB after creation:", tables)
        logger.info(f"Tables in DB after creation: {tables}")
        execute_sql_file(connection, SQL_CREATE_FOLDER.joinpath('03_project_insert_records.sql'))
